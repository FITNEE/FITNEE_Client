...
const ExerciseItem_Custom = ({ id, content, title, popMessage, editRoutine, isDark }) => {
  let isCardio = title == '러닝' || title == '사이클'
  return (
    <ExerciseContainer
...
export const List_Custom = ({ isDark, setNewSCHE, SCHEDULE, newRoutine, editRoutine, popMessage }) => {
  return (
    <ScrollPressable>
      ...
      <ComponentTitle title="운동 편집" subTitle="운동을 꾹 누르면 세부사항을 편집할 수 있어요" isDark={isDark} />
      {newRoutine ? (
        <>
        ...
        <ContentContainer>
          <NoRoutineText style={{ marginTop: 160, color: colors.grey_5 }}>해당 요일에는 루틴이 없어요</NoRoutineText>
        </ContentContainer>
      )}
    </ScrollPressable>
  )
