...
export default function Dictionary_RightTab(props) {
  ...
  // 참여하기 버튼
  const { parentJoinBtnBool, parentSetJoinBtnBool, exerciseName, leftTabActivate, setIsAllRead, handleSnapPress } =
    props
  ...
  const getChat = async () => {
    try {
      ...
      if (result.result.isSuccess) console.log(`채팅 불러오기 성공 (운동이름 : ${exerciseName})`)
  ...
  const putChatRead = async (idx) => {
    try {
      ...
      if (result.isSuccess) console.log(`어디까지 읽었는지 저장 성공 (idx : ${idx})`)
    ...
    <>
      {msg.length != 0 ? (
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <BottomSheetScrollView style={{ paddingTop: 28 }} showsVerticalScrollIndicator={false}>
            {msg.map((msg, i) => (
              <ChatContainer key={i}>
                {msg.userNickname != myNickName ? (
                  <MessageWrapper>
                    ...
                    <MessageContainer style={{ backgroundColor: isDark ? `${colors.grey_8}` : `${colors.grey_1}` }}>
                      <MessageText style={{ color: isDark ? `${colors.white}` : `${colors.black}` }}>
                        {msg.text}
                      </MessageText>
                    </MessageContainer>
                  </MessageWrapper>
      ...
      {childJoinBtnBool ? null : (
        <TextInputBG
          style={{
            backgroundColor: Platform.OS === 'ios' ? (isDark ? `#303235` : `#D1D3D9`) : isDark ? `#262626` : `#E3E2EA`,
