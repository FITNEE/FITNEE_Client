...
export default function TotalChart() {
    const [message, setMessage] = useState({ x:0, y:0, visible: false, value: 0});
    ...
    const data = {
        ...
        datasets: [
        { data: [4, 3, 7, 6, 2, 5, 6, 9], color: () => colors.l_main, strokeWidth: 4 },
        { data: [2, 5, 6, 9, 4, 3, 7, 6], color: () => colors.green, strokeWidth: 4 }
        ],
        };
        ...
        contentOffset={{x: (data.labels.length * screenWidth) / 8, y: 0}}>
            ...
            decorator={() => {
                ...
                style={{
                    ...
                    {<Text style={{fontSize: 11, fontWeight:500, color: colors.white, textAlign: "center"}}>
                        {" kcal"}
                    </Text>}
                </View>
            ...
            onDataPointClick={(data) => {
                if(isDataset0Larger){
                setMessage({ x: data.x, value: data.value, y: data.y, visible: true });
                }

                let isSamePoint = (message.x === data.x && message.y === data.y)

                isSamePoint ? setMessage((previousState) => {
                    return { 
                              ...previousState,
                              value: data.value,
