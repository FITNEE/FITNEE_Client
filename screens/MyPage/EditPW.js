...
const InputRed2 = styled.View`
  ...
  border-color: ${({ error, prevCheck, contents, on }) =>
    prevCheck && contents != '' && on ? (error ? colors.red : colors.green) : 'transparent'};
`
    ...
    <SafeAreaView backgroundColor={isDark ? colors.grey_9 : colors.grey_1}>
      ...
      >
        <Container isDark={isDark}>
            ...
            <InputRed1 error={errorPW}>
              <Input
                ...
                style={{
                  ...
                  color: checking ? (isDark ? colors.grey_7 : colors.grey_2) : isDark ? colors.white : colors.black,
                }}
                autoFocus
                ref={(input) => {
          ...
          {!checking ? (
            <Button onPress={() => checkPW()} enabled={rewrittenPW != ''} text={'확인'}></Button>
          ) : visibleErrorNewPW ? (
            <Button
            ...
            <Button onPress={() => checkNewPW()} enabled={newPW != ''} text={'확인'}></Button>
          )}
        </Container>
      </TouchableWithoutFeedback>
    </SafeAreaView>
