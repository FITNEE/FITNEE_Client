...
const Login = ({ route, navigation }) => {
  ...
  const postDeviceKey = async (deviceToken) => {
    try {
      let url = `https://gpthealth.shop/app/user/device?devToken=${FCMToken}`
      let data = { devToken: deviceToken }
      const response = await axios.put(url, data)
      console.log(response)
      const result = response.data.devToken
      return result
    } catch (error) {
